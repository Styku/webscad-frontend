  ...
  <main id="app">
    <div class="grid-main" >
      <div class="left-panel">
          ...
          <script-list v-bind:scripts="filtered_scripts" v-bind:selected_script="script" @selected="loadScript($event)"></script-list>
      </div>
      <div>
          <menu class="w3-theme-d4">
              <button @click.prevent="getPreview(script)" class="w3-button w3-blue">Preview</button>
              <button @click.prevent="getStl(script)" class="w3-button w3-green">Render</button>
          </menu>
          <script-input v-bind:script="script" v-bind:image_catalog="image_catalog" @changed="getPreview(script)"></script-input>
      </div>
    </div>
  </main>
</template>
